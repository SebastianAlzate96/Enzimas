# -*- coding: utf-8 -*-
"""
Created on Mon Mar 13 17:27:32 2023
...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 13 11:06:37 2023
...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 12 12:15:22 2023
...
def draw_bs_sample(data, rg=np.random.RandomState()):
    x_star = markovmodel_P.sample(n_samples=len(data[:,0]),random_state=rg)
    return(np.round(x_star[0],0))

#draw_bs_sample(X)


def compute_jackknife_reps(data=Y, statfunction=statfunction_means):
    ...
    jack_reps = np.empty((len(data),n_states*2))
...
#jack_reps = compute_jackknife_reps(Y)
...
def bootstrap_replicates(data, n_reps=10, statfunction=statfunction_means):
    '''Computes n_reps number of bootstrap replicates for given data and statistical function'''
    boot_reps = np.empty((n_reps,n_states*2))
    for i in range(n_reps):
        boot_reps[i,:] = statfunction(draw_bs_sample(data))
    return boot_reps

#boot_reps = bootstrap_replicates(X)
...
#3compute_z0(X, boot_reps)

def compute_bca_ci(data, alpha_level, n_reps, statfunction=statfunction_means):
...
BCa = compute_bca_ci(data = Y, alpha_level = 0.05, n_reps = 10)
